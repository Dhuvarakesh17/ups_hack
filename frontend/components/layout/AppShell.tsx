...
export function AppShell({ children, title = "Dashboard" }: AppShellProps) {
  ...
  useEffect(() => {
    const localToken = localStorage.getItem("session_token") || localStorage.getItem("better-auth.session_token");
    ...
    if (session?.user) {
      ...
      if (googleUser.email) localStorage.setItem("current_user_email", googleUser.email);
    ...
    return (
      <div className="min-h-screen bg-white dark:bg-[#17231b] flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-xs font-semibold text-slate-500 dark:text-[#9bb3a1]">Checking authentication...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen w-full bg-white dark:bg-[#17231b] overflow-hidden text-[#17231b] dark:text-[#edf7cd]">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        <Header title={title} onOpenMobileMenu={() => setIsMobileMenuOpen(true)} />
